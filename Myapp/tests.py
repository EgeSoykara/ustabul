...
class MarketplaceTests(TestCase):
    ...
    def test_notifications_view_respects_disabled_categories(self):
        ...
        self.assertContains(response, "Kapalı bildirim kategorileri")
        self.assertContains(response, "Talep")
    ...
    def test_my_requests_highlights_card_and_shows_recent_change_badge(self):
        ...
        ServiceMessage.objects.create(
            ...
            sender_user=self.provider_user_ali,
            sender_role="provider",
            body="Kartta yeni mesaj rozeti görünmeli",
        )
...
class MobileApiTests(TestCase):
    ...
    def test_mobile_notifications_list_mark_read_and_preferences(self):
        service_request = ServiceRequest.objects.create(
            customer_name="Mobile Bildirim Musteri",
            customer_phone="05001112233",
            city="Lefkosa",
            district="Ortakoy",
            service_type=self.service,
            details="Mobile notification test",
            customer=self.customer_user,
            matched_provider=self.provider,
            status="matched",
        )
        ...
        service_request.matched_at = timezone.now()
        service_request.save(update_fields=["matched_offer", "matched_at"])
        incoming_message = ServiceMessage.objects.create(
            service_request=service_request,
            sender_user=self.provider_user,
            sender_role="provider",
            body="Mobil bildirim mesajı",
        )
        ...
        self.assertTrue(preferences_response.json()["allow_message_notifications"])
        ...
        cursor = NotificationCursor.objects.get(user=self.customer_user)
        self.assertTrue(cursor.allow_message_notifications)
        self.assertFalse(cursor.allow_request_notifications)
        self.assertFalse(cursor.allow_appointment_notifications)
        ...
        self.assertEqual(results[0]["entry_id"], f"msg-{incoming_message.id}")
        self.assertEqual(results[0]["service_request_id"], service_request.id)

        read_response = self.client.post(
            f"/mobile/api/v1/notifications/msg-{incoming_message.id}/read/",
            HTTP_AUTHORIZATION=f"Bearer {access}",
        )
    ...
    def test_mobile_provider_dashboard_includes_pending_sections(self):
        ...
        ProviderOffer.objects.create(
            service_request=pending_request,
            provider=self.provider,
            token="MOBILEPENDING001",
            sequence=1,
            status="pending",
        )
        waiting_request = ServiceRequest.objects.create(
            customer_name="Secim Bekleyen",
            customer_phone="05003334455",
            city="Lefkosa",
            district="Ortakoy",
            service_type=self.service,
            details="Secim bekliyor",
            customer=self.customer_user,
            status="pending_customer",
        )
        ProviderOffer.objects.create(
            service_request=waiting_request,
            provider=self.provider,
            token="MOBILEWAIT001",
            sequence=1,
            status="accepted",
        ...
        active_request = ServiceRequest.objects.create(
            customer_name="Aktif Musteri",
            customer_phone="05004445566",
            city="Lefkosa",
            district="Ortakoy",
            service_type=self.service,
            details="Aktif thread",
            customer=self.customer_user,
            matched_provider=self.provider,
            status="matched",
        )
        active_offer = ProviderOffer.objects.create(
            service_request=active_request,
            provider=self.provider,
            token="MOBILEACTIVE001",
            sequence=1,
            status="accepted",
        )
        active_request.matched_offer = active_offer
        active_request.matched_at = timezone.now()
        active_request.save(update_fields=["matched_offer", "matched_at"])
        ...
        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertEqual(body["snapshot"]["pending_offers_count"], 1)
    ...
    def test_mobile_marketplace_bootstrap_returns_search_and_form_payload(self):
        payload = self._login_mobile("mobile_customer", "GucluSifre123!")
        access = payload["access"]
        ...
        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertTrue(body["user_context"]["can_create_request"])
        self.assertIn(
            self.service.name,
            [item["name"] for item in body["search"]["service_types"]],
        )
        self.assertIn("Lefkoşa", body["search"]["city_district_map"])
        self.assertEqual(body["request_form"]["initial"]["customer_name"], "mobile_customer")

    def test_mobile_marketplace_providers_filters_by_service_type(self):
        response = self.client.get(
            f"/mobile/api/v1/marketplace/providers/?service_type={self.service.id}&limit=10&offset=0"
        )

        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertEqual(body["count"], 1)
        self.assertEqual(body["results"][0]["full_name"], self.provider.full_name)

    def test_mobile_provider_detail_returns_reviews_and_stats(self):
        completed_request = ServiceRequest.objects.create(
            customer_name="Detay Musteri",
            customer_phone="05005556677",
            city="Lefkosa",
            district="Ortakoy",
            service_type=self.service,
        ...
        response = self.client.get(f"/mobile/api/v1/marketplace/providers/{self.provider.id}/")

        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertEqual(body["provider"]["full_name"], self.provider.full_name)
        self.assertEqual(body["completed_jobs"], 1)
        self.assertEqual(body["recent_ratings"][0]["comment"], "Harika hizmet")

    def test_mobile_customer_can_create_request_with_preferred_provider(self):
        payload = self._login_mobile("mobile_customer", "GucluSifre123!")
        access = payload["access"]
            ...
            data=json.dumps(
                {
                    ...
                    "city": "Lefkosa",
                    "district": "Ortakoy",
                    "details": "Sadece secilen ustaya gitmeli",
                    "preferred_provider_id": self.provider.id,
                }
            ),
            content_type="application/json",
            HTTP_AUTHORIZATION=f"Bearer {access}",
        ...
        self.assertEqual(response.status_code, 201)
        created_request = ServiceRequest.objects.get(customer=self.customer_user, details="Sadece secilen ustaya gitmeli")
        self.assertEqual(created_request.preferred_provider_id, self.provider.id)
        self.assertTrue(
            ProviderOffer.objects.filter(service_request=created_request, provider=self.provider).exists()
        )
        self.assertEqual(response.json()["request"]["request_code"], created_request.request_code)

    def test_mobile_request_detail_returns_customer_actions_and_offers(self):
        service_request = ServiceRequest.objects.create(
            customer_name="Secim Bekleyen Musteri",
            customer_phone="05001231234",
            city="Lefkosa",
            district="Ortakoy",
            service_type=self.service,
            details="Teklif secim detayi",
            customer=self.customer_user,
            status="pending_customer",
        )
        accepted_offer = ProviderOffer.objects.create(
            service_request=service_request,
            provider=self.provider,
            token="MOBILEDETAIL001",
            sequence=1,
            status="accepted",
            quote_note="Yarin sabah gelebilirim.",
            responded_at=timezone.now(),
        )
        ...
        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertEqual(body["viewer_role"], "customer")
        self.assertTrue(body["actions"]["can_select_offer"])
    ...
    def test_mobile_customer_can_select_offer(self):
        service_request = ServiceRequest.objects.create(
            customer_name="Teklif Secen Musteri",
            customer_phone="05002224466",
            city="Lefkosa",
            district="Ortakoy",
            service_type=self.service,
            details="Teklif secimi",
            customer=self.customer_user,
            status="pending_customer",
        )
        accepted_offer = ProviderOffer.objects.create(
        ...
        access = payload["access"]
        response = self.client.post(
            f"/mobile/api/v1/customer/requests/{service_request.id}/offers/{accepted_offer.id}/select/",
            data=json.dumps({}),
            content_type="application/json",
            HTTP_AUTHORIZATION=f"Bearer {access}",
        )
        ...
        service_request.refresh_from_db()
        self.assertEqual(service_request.status, "matched")
        self.assertEqual(service_request.matched_provider_id, self.provider.id)
        self.assertEqual(service_request.matched_offer_id, accepted_offer.id)

    def test_mobile_provider_can_accept_offer(self):
        service_request = ServiceRequest.objects.create(
            customer_name="Teklif Bekleyen Musteri",
            customer_phone="05006667788",
            city="Lefkosa",
        ...
        access = payload["access"]
        response = self.client.post(
            f"/mobile/api/v1/provider/offers/{pending_offer.id}/accept/",
            data=json.dumps({"quote_note": "Aksam gelebilirim."}),
            content_type="application/json",
            HTTP_AUTHORIZATION=f"Bearer {access}",
        )
        ...
        service_request.refresh_from_db()
        self.assertEqual(pending_offer.status, "accepted")
        self.assertEqual(pending_offer.quote_note, "Aksam gelebilirim.")
        self.assertEqual(service_request.status, "pending_customer")

    def test_mobile_provider_can_confirm_appointment(self):
        service_request = ServiceRequest.objects.create(
            customer_name="Randevu Bekleyen Musteri",
            customer_phone="05007778899",
            city="Lefkosa",
            district="Ortakoy",
            service_type=self.service,
            details="Randevu onayi",
            customer=self.customer_user,
            matched_provider=self.provider,
            status="matched",
        )
        ...
        appointment = ServiceAppointment.objects.create(
            ...
            provider=self.provider,
            scheduled_for=timezone.now() + timedelta(hours=2),
            customer_note="Saat uygunsa onaylayin.",
            status="pending",
        )
        ...
        access = payload["access"]
        response = self.client.post(
            f"/mobile/api/v1/provider/appointments/{appointment.id}/confirm/",
            data=json.dumps({"provider_note": "Saat uygun."}),
            content_type="application/json",
            HTTP_AUTHORIZATION=f"Bearer {access}",
        )
