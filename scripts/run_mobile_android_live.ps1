...
$siteUrl = "https://ustabul.onrender.com"
...
try {
    ...
    if ($response.StatusCode -lt 200 -or $response.StatusCode -ge 400) {
        throw "Live site did not return a healthy response: $($response.StatusCode)"
    }
    ...
    flutter pub get
    flutter run -d $deviceId --dart-define=SITE_URL=$siteUrl --no-resident
}
finally {
    Pop-Location
}
